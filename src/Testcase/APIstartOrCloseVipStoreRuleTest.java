...
public class APIstartOrCloseVipStoreRuleTest extends APITest{
	  @Test
	  public void startOrCloseVipStoreRuleTest() throws Exception {
		  
		  boolean apiResult=true;
			String[][] testMap=ReadExcelCase.getTestMap("/vipSettingWS/startOrCloseVipStoreRule");
			for(int i=0;i<testMap.length;i++){
				  ...
				  try {
					  ...
					  db1.pst.executeUpdate();
//					  //商户283添加未删除的储值规则
//					  db1.pst=db1.conn.prepareStatement("insert into vip_store_rule (id,tenant_id,branch_id,is_off,dead_type,is_deleted)values(100,283,13895,0,0,0)");
//					  db1.pst.executeUpdate();
					  ...
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("insert into vip_store_rule_details (id,rule_id,pay_limit,present_limit,is_deleted)values(8002,102,100,2,0)");
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("insert into vip_store_rule_details (id,rule_id,pay_limit,present_limit,is_deleted)values(8003,102,200,5,1)");
					  db1.pst.executeUpdate();
					db1.close();
