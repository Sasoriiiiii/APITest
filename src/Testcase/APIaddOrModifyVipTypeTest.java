...
public class APIaddOrModifyVipTypeTest extends APITest{
	  @Test
	  public void addOrModifyVipTypeTest() throws Exception {
		  
		  boolean apiResult=true;
			String[][] testMap=ReadExcelCase.getTestMap("/vipSettingWS/addOrModifyVipType");
			for(int i=0;i<testMap.length;i++){
				  ...
				  try {
					  ...
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("update vip_type set id=8002 where type_name='普通会员' and tenant_id=243");
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("INSERT INTO vip_type (tenant_id, type_code, type_name, preferential_policy, points_factor,score_usage,mem_price_used, "
						  		+ "discount_rate, to_save_points, is_package_disc, is_promotion_disc, create_by, create_at, last_update_by, last_update_at, is_deleted,is_online_default)"
						  		+ "VALUES('243', '0003','修改_cws','2','1.00','1','1.000','1','1','1','1','admin',NOW(),'admin',NOW(),'0','0')");
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("update vip_type set id=8000 where type_name='修改_cws' and tenant_id=243");
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("INSERT INTO vip_type (tenant_id, type_code, type_name, preferential_policy, points_factor,score_usage,mem_price_used, "
							  		+ "discount_rate, to_save_points, is_package_disc, is_promotion_disc, create_by, create_at, last_update_by, last_update_at, is_deleted,is_online_default)"
							  		+ "VALUES('243', '0004','删除会员','2','10.00','10','1.000','1','1','1','1','admin',NOW(),'admin',NOW(),'1','0')");
					  db1.pst.executeUpdate();
					  db1.pst=db1.conn.prepareStatement("update vip_type set id=8001 where type_name='删除会员' and tenant_id=243");
					  db1.pst.executeUpdate();
