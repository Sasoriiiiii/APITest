...
public class APIisEnablePaymentTest extends APITest{
	@Test
	public void isEnablePaymentTest() throws Exception {

		boolean apiResult = true;
		String[][] testMap = ReadExcelCase.getTestMap("/baseWS/isEnablePayment");
		for (int i = 0; i < testMap.length; i++) {
			...
			try {
				...
				db1.pst = db1.conn.prepareStatement(
						"insert into payment (id,payment_code,payment_name,payment_status,currency_id,is_deleted,tenant_id,branch_id,payment_type)"
								+ "values(8009,'ZDY_0001','zf2',0,1,0,243,13892,1)");
				db1.pst.executeUpdate();
				db1.pst = db1.conn.prepareStatement(
						"insert into payment (id,payment_code,payment_name,payment_status,currency_id,is_deleted,tenant_id,branch_id,payment_type)"
								+ "values(8010,'ZDY_0001','zf3',1,1,0,243,13892,1)");
				db1.pst.executeUpdate();
				db1.close();
